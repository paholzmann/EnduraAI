...
class UTMBFeatures:
    ...
    def calculate_race_result_features(self, utmb_df: pd.DataFrame) -> pd.DataFrame:
        ...
        def calculate_features(results):
            ...
            top_10_percent_average_time = np.mean(
                top_10_values) if top_10_values else np.nan
            podium_average_time = np.mean(heapq.nsmallest(
                3, clean_results)) if len(clean_results) >= 3 else np.nan
            podium_vs_field = (podium_average_time / median_time if pd.notna(
                podium_average_time) and pd.notna(median_time) and median_time != 0 else np.nan)
            return (
                winning_time,
                median_time,
                slowest_time,
    ...
    def calculate_normalized_features(self, utmb_df: pd.DataFrame) -> pd.DataFrame:
        ...
        utmb_df["Winner_Pace"] = (
            utmb_df["Winning_Time"] / utmb_df["Distance"])
        utmb_df["Effort_Adjusted_Median_Time"] = (
            utmb_df["Median_Time"] / utmb_df["Race_Effort"])
        utmb_df["Effort_Adjusted_Winning_Time"] = (
            utmb_df["Winning_Time"] / utmb_df["Race_Effort"])
        return utmb_df
    ...
    def calculate_competition_features(self, utmb_df: pd.DataFrame) -> pd.DataFrame:
        ...
        utmb_df = utmb_df.copy()
        utmb_df["Elite_Gap"] = (
            utmb_df["Top_10_Percent_Average_Time"] / utmb_df["Winning_Time"])
        utmb_df["Podium_Gap"] = (
            utmb_df["Podium_Average_Time"] / utmb_df["Winning_Time"])
        utmb_df["Depth_Of_Field"] = (
            utmb_df["Median_Time"] / utmb_df["Winning_Time"])
        utmb_df["Top_vs_Median"] = (
            utmb_df["Top_10_Percent_Average_Time"] / utmb_df["Median_Time"])
        return utmb_df
    ...
    def calculate_difficulty_features(self, utmb_df: pd.DataFrame) -> pd.DataFrame:
        ...
        utmb_df = utmb_df.copy()
        utmb_df["Elevation_per_km"] = (
            utmb_df["Elevation_Gain"] / utmb_df["Distance"])
        utmb_df["Effort_per_km"] = (
            utmb_df["Race_Effort"] / utmb_df["Distance"])
